...
class ProfilesPage(QtWidgets.QWidget):
    ...
    def setupUi(self, profilespage):
        ...
        self.shipping_card.setStyleSheet(
        ...
        font.setPointSize(18) if platform.system() == "Darwin" else font.setPointSize(
            18 * 0.75
        )
        font.setBold(False)
        font.setWeight(50)
        ...
        self.billing_card.setStyleSheet(
            "background-color: #232323;border-radius: 20px;border: 1px solid #2e2d2d;"
        )
        ...
        self.same_shipping_checkbox.setStyleSheet(
            "border:none;color: rgb(234, 239, 239);"
        )
        self.same_shipping_checkbox.setText("Same as shipping")
        self.same_shipping_checkbox.stateChanged.connect(
            self.same_shipping_checkbox_clicked
        )

        # Payment window
        self.tasks_card_3 = QtWidgets.QWidget(self.profilespage)
        self.tasks_card_3.setGeometry(QtCore.QRect(700, 70, 313, 501))
        self.tasks_card_3.setStyleSheet(
            "background-color: #232323;border-radius: 20px;border: 1px solid #2e2d2d;"
        )
    ...
    def set_data(self):
        ...
        for month in range(1, 13):
            self.cardmonth_box.addItem(
                str(month)
            ) if month > 9 else self.cardmonth_box.addItem("0" + str(month))
    ...
    def same_shipping_checkbox_clicked(self):
        if self.same_shipping_checkbox.isChecked():
            self.billing_country_box.setCurrentIndex(
                self.billing_country_box.findText(
    ...
    def save_profile(self):
        ...
        profile_data = {
            ...
            "card_number": (Encryption().encrypt(self.cardnumber_edit.text())).decode(
                "utf-8"
            ),
    ...
    def delete_profile(self):
        ...
        self.parent().parent().createdialog.profile_box.removeItem(
            self.parent().parent().createdialog.profile_box.findText(profile_name)
        )
