...
class Walmart:
                    ...
                    if "Add to Cart" in r.text:
                        if self.max_price != "":
                            if float(self.max_price) < price:
                                ...
                                continue
                        offer_id = json.loads(
                            doc.xpath('//script[@id="item"]/text()')[0]
                        )["item"]["product"]["buyBox"]["products"][0]["offerId"]
    ...
    def check_cart_items(self):
        ...
        while True:
            ...
            try:
                r = self.session.post(
                    "https://www.walmart.com/api/checkout/v3/contract?page=CHECKOUT_VIEW",
                    json=body,
                    headers=headers,
                )
                if r.status_code == 201:
                    ...
                    self.status_signal.emit(
                        {"msg": "Loaded Cart Items", "status": "normal"}
                    )
                    ...
                    if json.loads(r.text)["message"] == "Item is no longer in stock.":
                        ...
                        self.status_signal.emit(
                            {
                                "msg": "Error Loading Cart Items, Got Response: "
                                + str(r.text),
                                "status": "error",
                            }
                        )
                ...
                self.status_signal.emit(
                    {
                        "msg": "Error Loading Cart Items (line {} {} {})".format(
                            sys.exc_info()[-1].tb_lineno, type(e).__name__, e
                        ),
                        "status": "error",
                    }
    ...
    def submit_shipping_method(self, item_id, fulfillment_option, ship_method):
        ...
        while True:
            self.status_signal.emit(
                {"msg": "Submitting Shipping Method", "status": "normal"}
            )
            try:
                r = self.session.post(
                    "https://www.walmart.com/api/checkout/v3/contract/:PCID/fulfillment",
                    json=body,
                    headers=headers,
                )
                if r.status_code == 200:
                    try:
                        r = json.loads(r.text)
                        self.status_signal.emit(
                            {"msg": "Submitted Shipping Method", "status": "normal"}
                        )
                        return
                    except:
                        pass
                self.status_signal.emit(
                    {"msg": "Error Submitting Shipping Method", "status": "error"}
                )
                time.sleep(self.error_delay)
            except Exception as e:
                self.status_signal.emit(
                    {
                        "msg": "Error Submitting Shipping Method (line {} {} {})".format(
                            sys.exc_info()[-1].tb_lineno, type(e).__name__, e
                        ),
                        "status": "error",
                    }
    ...
    def submit_shipping_address(self):
        ...
        while True:
            self.status_signal.emit(
                {"msg": "Submitting Shipping Address", "status": "normal"}
            )
            try:
                r = self.session.post(
                ...
                if r.status_code == 200:
                    try:
                        r = json.loads(r.text)
                        self.status_signal.emit(
                            {"msg": "Submitted Shipping Address", "status": "normal"}
                        )
                        return
                    except:
                        pass
                self.status_signal.emit(
                    {"msg": "Error Submitting Shipping Address", "status": "error"}
                )
                time.sleep(self.error_delay)
            except Exception as e:
                self.status_signal.emit(
                    {
                        "msg": "Error Submitting Shipping Address (line {} {} {})".format(
                            sys.exc_info()[-1].tb_lineno, type(e).__name__, e
                        ),
                        "status": "error",
                    }
    ...
    def get_PIE(self):
        ...
        while True:
            self.status_signal.emit(
                {"msg": "Getting Checkout Data", "status": "normal"}
            )
            try:
                r = self.session.get(
                    "https://securedataweb.walmart.com/pie/v1/wmcom_us_vtg_pie/getkey.js?bust="
                    + str(int(time.time())),
                    headers=headers,
                )
                if r.status_code == 200:
                    ...
                    self.status_signal.emit(
                        {"msg": "Got Checkout Data", "status": "normal"}
                    )
                    return card_data, PIE_key_id, PIE_phase
                self.status_signal.emit(
                    {"msg": "Error Getting Checkout Data", "status": "error"}
                )
                time.sleep(self.error_delay)
            except Exception as e:
                self.status_signal.emit(
                    {
                        "msg": "Error Getting Checkout Data (line {} {} {})".format(
                            sys.exc_info()[-1].tb_lineno, type(e).__name__, e
                        ),
                        "status": "error",
                    }
            ...
            try:
                ...
                if r.status_code == 200:
                    try:
                        r = json.loads(r.text)
                        self.status_signal.emit(
                            {"msg": "Submitted Billing", "status": "normal"}
                        )
                        return
                    except:
                        pass
                self.status_signal.emit(
                    {"msg": "Error Submitting Billing", "status": "error"}
                )
                if self.check_browser():
                    return
                time.sleep(self.error_delay)
                ...
                self.status_signal.emit(
                    {
                        "msg": "Error Submitting Billing (line {} {} {})".format(
                            sys.exc_info()[-1].tb_lineno, type(e).__name__, e
                        ),
                        "status": "error",
                    }
    ...
    def submit_order(self):
        ...
        while True:
            ...
            try:
                r = self.session.put(
                    "https://www.walmart.com/api/checkout/v3/contract/:PCID/order",
                    json={},
                    headers=headers,
                )
                try:
